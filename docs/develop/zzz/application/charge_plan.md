...
- **多倍活动**：开启后在每次运行首次打开快捷手册时，依次检查实战模拟室和区域巡防的双倍、三倍活动；命中时生成临时计划优先执行，不写入正式计划列表。
...
- 开启多倍活动时，首次识别资源栏后依次检查两个栏目，命中第一个活动后生成一条临时计划优先执行；后续 skip 循环不再重复检查
- 实战模拟室与区域巡防的活动横幅使用同一识别区域，但文案不同；脚本会识别文字中的双倍或三倍以及剩余次数
- 活动倍数只影响奖励份数，不改变单次电量消耗；实战模拟室仍按每张怪物卡 20 电量计算，区域巡防仍按每次 60 电量计算
- 实战模拟室继续在一次挑战中选择对应数量的怪物卡；区域巡防在一条临时计划中按计算次数使用「再来一次」，最后一次才点「完成」
